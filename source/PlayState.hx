...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (FlxG.save.data.ghostTapping || FlxG.save.data.autoplay)
			Conductor.safeFrames = Conductor.safeFrames;
		else
...
if(FlxG.save.data.antialiasing)
		...
		if (SONG.player2 == 'dad')
			{
				iconP2.y = healthBar.y - (iconP2.height / 2) - 20;
			}
			else
				{
...
hudGroup.add(scoreTxt);
	...
	override public function update(elapsed:Float)
	{
		...
		iconP1.x = FlxG.width / 1.36;
		iconP2.x = FlxG.width / 7;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (FlxG.save.data.changedHit)
			{
				rating.x = FlxG.save.data.changedHitX + 70;
				rating.y = FlxG.save.data.changedHitY + 20;
			}
		...
		if (FlxG.save.data.changedHitcombo)
			{
				comboSpr.x = FlxG.save.data.changedHitcomboX + 20;
				comboSpr.y = FlxG.save.data.changedHitcomboY + 15;
			}
					...
					if (possibleNotes[0].strumTime == possibleNotes[1].strumTime)
					{
						for (coolNote in possibleNotes)
						{
							...
							else
							{
								...
								if (!inIgnoreList)
									(FlxG.save.data.ghostTapping || FlxG.save.data.autoplay ? "" : badNoteCheck());
							}
						}
					}
		...
		notes.forEachAlive(function(daNote:Note)
            {
                ...
                !FlxG.save.data.downscroll && daNote.y < strumLine.y)
                {
                    ...
                    if(FlxG.save.data.autoplay && daNote.canBeHit && daNote.mustPress ||
                        FlxG.save.data.autoplay && daNote.tooLate && daNote.mustPress)
                    {
		...
		playerStrums.forEach(function(spr:FlxSprite)
		{
			switch (spr.ID)
			{
				case 0:
					...
					if (start == false || FlxG.save.data.autoplay)
					spr.animation.play('static');
					else
